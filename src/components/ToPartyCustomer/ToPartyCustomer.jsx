...
const ToPartyCustomer = () => {
  ...
  const currentRecords = toParties.slice(indexOfFirstRecord, indexOfLastRecord);
  const totalPages = Math.ceil(toParties.length / recordsPerPage);
  ...
  const handleSave = () => {
   fetch(`${API_BASE_URL}/api/tp/customers`, {
         method: "POST",
         headers: {
           "Content-Type": "application/json",
         },
         body: JSON.stringify(newToParty), // Convert the JavaScript object to JSON
       })
         .then((response) => {
           if (!response.ok) {
    ...
    console.log("Saving product:", newToParty);
    closeModal();
  };
  ...
  return (
    <div className="ledger-container p-0">
      ...
      <table className="ledger-table table table-bordered">
        ...
        <tbody>
          {currentRecords.length === 0 ? (
            ...
            toParties.map((party, index) => (
              <tr key={index}>
                <td>{party.tpCustomerId}</td>
      ...
      {/* Pagination */}
      {toParties.length > recordsPerPage && (
...
export default ToPartyCustomer;
